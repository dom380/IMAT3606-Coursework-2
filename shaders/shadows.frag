...
{ 
	...
	{ 
		vec3 ambient = lights[i].ambient * material.Ka * vec3(1,1,1); 
		 
		//diffuse term 
		vec3 lightDir = normalize(lights[i].pos - inData.FragPos); 
		float diff = max(dot(inData.Normal, lightDir), 0.0); 
		vec3 diffuse = diff * lights[i].diffuse * material.Kd * vec3(1,1,1); 
		...
		vec3 specular = lights[i].specular * spec * material.Ks * vec3(1,1,1); 
